...
export default function Profile() {
  const { appState, modules, getModuleProgress } = useAppContext();
  ...
  const completedCount = Object.values(appState.moduleProgress).filter((p) => p.passed).length;
  ...
  return (
    <div>
      <h2 style={{ color: "#fff", marginBottom: 20, fontSize: "1.4rem" }}>👤 Your Profile</h2>
      <div className="card">
        <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20 }}>
          <div className="avatar" style={{ width: 60, height: 60, fontSize: "1.5rem" }}>{initials}</div>
          <div>
            <h3 style={{ color: "#fff" }}>{user?.displayName || "Agent"}</h3>
            <p style={{ color: "#8aa4c0", fontSize: "0.85rem", marginBottom: 4 }}>@{user?.username || "unknown"}</p>
            ...
            <div key={mod.id} style={{ padding: "12px 16px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 8, marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span style={{ color: "#8aa4c0", fontSize: "0.85rem" }}>
                  Best: {progress.bestScore} pts | Attempts: {progress.quizAttempts}
                </span>
                {progress.passed ? (
